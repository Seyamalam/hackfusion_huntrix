  ...
  return (
    ...
    >
      <AnimatedPanel index={0}>
        <HeroBanner
          eyebrow="Module 1"
          title="Offline identity checkpoint"
        ...
        >
          <StatusPill label={state.role.replaceAll('_', ' ')} tone="info" />
          <StatusPill label={auth.compromised ? 'Tampered' : 'Audit Intact'} tone={auth.compromised ? 'alert' : 'success'} />
          <StatusPill label={`Device ${state.deviceId.slice(0, 8)}`} tone="neutral" />
        </HeroBanner>
      </AnimatedPanel>
      ...
      <AnimatedPanel index={2}>
        <OtpCard
          title="Offline TOTP"
          code={snapshot.code}
          detail={`Expires in ${snapshot.remainingSeconds}s and auto-regenerates every ${snapshot.periodSeconds}s.`}
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
            {auth.availableRoles.map((role) => (
              <Pressable
                ...
                style={{
                  borderRadius: 999,
                  backgroundColor: state.role === role ? '#f7d7cf' : palette.surfaceStrong,
      ...
      <AnimatedPanel index={5}>
        ...
        >
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
            <ActionChip label="Valid Login" onPress={auth.runValidLogin} tone="primary" />
            <ActionChip label="Invalid Login" onPress={auth.runInvalidLogin} />
            <ActionChip label="Tamper Log" onPress={auth.tamperChain} tone="danger" />
            <ActionChip label="Verify Chain" onPress={auth.verifyChain} />
          </View>
      ...
      <AnimatedPanel index={6}>
        ...
        >
          <AuthLogCard entries={[...state.auditLog].reverse().slice(0, 6)} />
        </SectionCard>
      </AnimatedPanel>
    </ScrollView>
  );
